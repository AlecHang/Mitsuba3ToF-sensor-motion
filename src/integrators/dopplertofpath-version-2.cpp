...
// version 2: 考虑了传感器运动的物理效应建模，且进行运动补偿
...
public:
    ...
    // dopplettofpath.cpp 中 eval_modulation_weight 修正版本
    Float eval_modulation_weight(Float ray_time, Float path_length, 
                                const Vector3f &sensor_dir) const {
        static constexpr Float c_m_per_s = 299792458.0f; // 精确光速
        
        // 角频率 (rad/s)
        Float w_g = 2 * M_PI * m_illumination_modulation_frequency_mhz * 1e6f;
        Float w_d = 2 * M_PI / m_time * m_hetero_frequency;
        
        // 飞行时间（秒）
        Float tof = path_length / c_m_per_s;
        
        // 传感器在ToF期间的位移及其在光线方向的投影
        ...
        // 相位项 φ = w_g * path_length_corr / c （与论文公式对齐）
        Float phi = w_g * path_length_corr / c_m_per_s;
        ...
        if (m_low_frequency_component_only) {
            ...
            // 全信号模式：包含高低频分量
            Float t1 = w_g * ray_time - phi; // 照明调制
            Float t2 = (w_g + w_d) * ray_time + m_sensor_modulation_phase_offset; // 传感器调制
